// app/pacientes-ficha/[id]/page.tsx
'use client'; 
...
// ----- ¡CORRECCIÓN DE RUTAS! -----
// Todas las importaciones usan el alias corregido @/app/...
import MainLayout from '@/app/components/Layout/MainLayout';
import { getPacienteById, deletePaciente } from '@/app/services/paciente.service';
import { type Paciente } from '@/app/services/paciente.service';
// ---------------------------------

import { Loader2, PawPrint, User, Home, Edit2, Trash2, ArrowLeft, ClipboardList, CalendarCheck } from 'lucide-react';

// --- Tipos de Datos ---
// 'Paciente' se importa desde el servicio
// --- Fin de Tipos ---
...
export default function FichaPacientePage() {
  ...
  const loadPaciente = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      // El servicio ya incluye el token
      const data: Paciente = await getPacienteById(id as string); 
      setPaciente(data);
    } catch (err: any) {
      console.error('Error al cargar la ficha del paciente:', err);
      setError(err.message || "No se pudo cargar la ficha médica.");
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const handleEliminar = async () => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar a ${paciente?.nombre}? Esta acción no se puede deshacer.`)) {
      try {
        await deletePaciente(id as string);
        alert('Paciente eliminado exitosamente.');
        router.push('/pacientes-listado'); // Redirigir al listado
      } catch (err: any) {
    ...
    return (
      <MainLayout>
         <div className="text-center py-12 bg-gray-50 border border-gray-200 rounded-lg">
            <p className="text-xl text-red-600 font-semibold">Paciente no encontrado.</p>
            <Link 
      ...
      <div className="max-w-5xl mx-auto bg-white p-8 rounded-xl shadow-2xl">
        ...
        <div className="flex items-start justify-between mb-8 border-b pb-4">
          ...
          <div className="flex space-x-3">
            ...
            {/* Botón Editar */}
            <Link 
              href={`/pacientes-editar/${id}`} // Asumiendo que tienes esta ruta
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Columna 2: Propietario Asociado */}
          <section className="lg:col-span-2 space-y-4 p-6 bg-white border border-gray-200 rounded-lg shadow-md">
            ...
            {paciente.propietario ? (
                <div>
                    ...
                    <Link 
                      href={`/propietarios/${paciente.propietario.id}`} 
                      className="mt-3 inline-block text-sm text-indigo-600 hover:text-indigo-800 font-medium transition duration-150"
                    >
        ...
        <section className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="space-y-6">
            ...
            {/* Aquí iría un componente de listado de Citas */}
